...
class sim():
    def __init__(self, rate=20, step_per=10, verbose=False):
        ...
        try:
            ...
            self.pub_position = rospy.Publisher("/sim_position", Pose2D, queue_size=10)
    ...
    def randomize(self):
        ...
        """
        point = [random.uniform(-3.35, 3.25), random.uniform(-3.25, 3.25), random.uniform(0, 2*np.pi)]
        print(f"new point: [{point[0]:0.3}, {point[1]:0.3}, {point[2]:0.3}]")
        self.reset(*point)
    ...
    def new_goal(self):
        """
            generates new goal
        """
        new_goal = np.array([random.uniform(-3.75, 3.75), random.uniform(-3.75, 3.75)])
